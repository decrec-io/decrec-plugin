...
| About to reopen a **Decision-worthy** direction (no `DEC-n`) | **Recall** → `search_decisions` → then reason |
...
2. If admitting a pick: synthesis (see **Tool Parameters**); include all three `roles` only if role essays already exist in thread — otherwise omit `roles`. Never partial `roles`. Skip **Explore** only when admitting without an explore ask and a preferred option plus ≥2 distinct alternatives already exist — then map those into `preferredOption` / `optionsConsidered` (prefer 2–3, max 4; option `summary`/`tradeoffs` from the thread). Never invent a pick just to unlock admit. Never run **Explore** only to fill `roles`. Remaining required strings: `confidence` = `medium` if they stated the pick firmly, else `low`; `tradeoffs` / `unresolvedQuestions` / `disagreementHighlights` from the thread, else `"unspecified"` — do not invent disagreements.
...
5. Show draft; **stop** unless also asked to admit. Do not implement the pick unless they asked — exploring or recording is not a request to implement it. When admitting after Explore: new DEC → **Create** with `exploration`; existing DEC, brief changed → **Update** with `exploration`; existing DEC, same brief → **Follow-up**. Send `synthesis` + all three `roles` (never partial).
...
Report `email`, `orgName`, and `auth` (`pat` | `oauth` | `session`).

## Report

Keep `briefVersion`, `expectedDecisionVersion`, and `explorationRunId` in this session for later tool calls. Do **not** paste them into the chat. Do not dump `stage`/`status` enums, run UUIDs, or “Next (UI only)” / “Next (this skill)” menus.
...
- One optional next step only if it helps (e.g. open the URL to rename or attach evidence). Title, evidence, and cancel in-flight explore have no tools — those are at the `url`. Do not list skill verbs.
